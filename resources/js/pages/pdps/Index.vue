...
const breadcrumbsItems = computed<BreadcrumbItem[]>(() => [
    {
        title: activeTab.value === 'Annex' ? 'Annex List' : 'PDP List',
        href: activeTab.value === 'Annex' ? '/pdps?tab=annex' : '/pdps',
    },
]);
...
watch(collapseOwned, (v) => {
    try {
        localStorage.setItem('pdp.collapseOwned', v ? '1' : '0');
    } catch {}
});
watch(collapseShared, (v) => {
    try {
        localStorage.setItem('pdp.collapseShared', v ? '1' : '0');
    } catch {}
});
...
const unseenByCriterion = ref<
    Record<string, { ids: number[]; types: UnseenType[] }>
>({});
...
const unseenByPdp = ref<
    Record<number, { approved: number; commented: number }>
>({});

const showPdpModal = ref(false);
const editingPdpId = ref<number | null>(null);
const pdpForm = reactive<Pdp>({
    id: 0,
    title: '',
    description: '',
    priority: 'Medium',
...
const skillForm = reactive<PdpSkill>({
    id: 0,
    pdp_id: 0,
    skill: '',
    description: '',
...
async function openProgressModal(s: PdpSkill, index: number) {
    ...
    try {
        ...
        if (bucket && Array.isArray(bucket.ids) && bucket.ids.length) {
            await Promise.all(
                bucket.ids.map((id) =>
                    http(`/notifications/${id}/read`, { method: 'POST' }).catch(
                        () => null,
                    ),
                ),
            );
...
async function loadProgressEntries() {
    ...
    try {
        const data = await http(
            `/pdps/${progressState.pdp_id}/skills/${progressState.skill_id}/criteria/${progressState.index}/progress.json`,
        );
        ...
        for (const it of progressState.entries) {
            if (it && typeof it.id === 'number') {
                progressState.commentDrafts[it.id] = String(
                    it.curator_comment || '',
                );
...
async function deleteProgressEntry(id: number) {
    ...
    try {
        await http(
            `/pdps/${progressState.pdp_id}/skills/${progressState.skill_id}/criteria/${progressState.index}/progress/${id}.json`,
            {
                method: 'DELETE',
            },
        );
...
async function toggleCriterionDone(s: PdpSkill, index: number, done: boolean) {
    try {
        await http(
            `/pdps/${s.pdp_id}/skills/${s.id}/criteria/${index}/done.json`,
            {
                method: 'PATCH',
        ...
        notifyError(
            'Failed to update criterion state: ' + (e?.message || 'Error'),
        );
    }
}
...
async function saveCuratorComment(id: number) {
    try {
        ...
        await http(
            `/pdps/${progressState.pdp_id}/skills/${progressState.skill_id}/criteria/${progressState.index}/progress/${id}/comment.json`,
            {
                method: 'PATCH',
...
async function approveProgressEntry(id: number) {
    try {
        await http(
            `/pdps/${progressState.pdp_id}/skills/${progressState.skill_id}/criteria/${progressState.index}/progress/${id}/approve.json`,
            {
                method: 'POST',
            },
        );
...
async function buildAnnexPdf(JsPDFCtor: any, data: any) {
    ...
    const owner =
        (data && data.owner) || (selectedPdp?.value as any)?.user || null;
    const ownerLine = owner
        ? `Owner: ${owner.name || owner.email || '—'}`
        : selectedPdpIsOwner.value
          ? 'Owner: You'
          : '';
    const curatorsList = Array.isArray(data && data.curators)
        ? (data.curators as any[])
        : curators.value;
    const curatorsLine =
...
async function downloadCurrentPdpAnnex() {
    ...
    if (
        selectedPdpIsOwner.value &&
        selectedPdpId.value &&
        curators.value.length === 0
    ) {
        try {
            await loadCurators(selectedPdpId.value);
        } catch {}
    }
    const payload = {
        ...(annex.value || {}),
        owner:
            (selectedPdp.value as any)?.user ||
            (selectedPdpIsOwner.value ? { name: 'You' } : null),
        curators: curators.value,
    };
...
async function saveSkill() {
    ...
    if (!skillForm.skill.trim()) {
        notifyError('Please fill the "Skill to achieve" field');
        return;
    }
    ...
    if (editingSkillId.value) {
        await http(
            `/pdps/${selectedPdpId.value}/skills/${editingSkillId.value}.json`,
            { method: 'PUT', body },
        );
    } else {
        await http(`/pdps/${selectedPdpId.value}/skills.json`, {
            method: 'POST',
            body,
        });
    }
...
async function deleteSkill(id: number) {
    if (!selectedPdpId.value) return;
    {
        const ok = await confirmDialog('Delete this skill?');
        if (!ok) return;
    }
    await http(`/pdps/${selectedPdpId.value}/skills/${id}.json`, {
        method: 'DELETE',
    });
    await loadSkills(selectedPdpId.value);
}
...
const selectedPdp = computed(() => {
    ...
    if (!id) return null;
    return (
        pdps.value.find((p) => p.id === id) ||
        sharedPdps.value.find((p) => p.id === id) ||
        null
    );
});
...
const selectedPdpIsEditable = computed(
    () => selectedPdpIsOwner.value || selectedPdpIsCurator.value,
);
...
watch(curatorEmail, (v) => {
    ...
    userSearchTimer = window.setTimeout(async () => {
        try {
            const data = await http(
                '/users.search.json?q=' + encodeURIComponent(v),
            );
...
async function assignCurator() {
    ...
    try {
        const res = await http(
            `/pdps/${selectedPdpId.value}/assign-curator.json`,
...
async function removeCurator(c: Curator) {
    ...
    try {
        await http(`/pdps/${selectedPdpId.value}/curators/${c.id}.json`, {
            method: 'DELETE',
        });
...
async function shareToUser() {
    if (!selectedPdpId.value) return;
    if (!selectedPdpIsOwner.value) {
        notifyError('Only the owner can share the PDP');
        return;
    }
    const email = curatorEmail.value.trim();
    ...
    try {
        if (!userId) {
            const exact = userOptions.value.find(
        ...
        if (!userId && email) {
            const data = await http(
                '/users.search.json?q=' + encodeURIComponent(email),
            );
        ...
        if (!userId) {
            notifyError('Select a user from the dropdown to share');
            return;
        }

        await http(`/pdps/${selectedPdpId.value}/transfer.json`, {
            method: 'POST',
...
function parseUnseenType(t: string | undefined | null): UnseenType | null {
    ...
    if (v.includes('approve') || v === 'success' || v === 'approved')
        return 'approved';
    if (v.includes('reject') || v === 'warning' || v === 'rejected')
        return 'rejected';
    ...
    try {
        ...
        for (const n of arr) {
            ...
            try {
                ...
                const criterion = parseInt(
                    u.searchParams.get('criterion') || '',
                    10,
                );
    ...
    <AppLayout :breadcrumbs="breadcrumbsItems">
        ...
        >
            <Heading
                ...
                :description="
                    activeTab === 'Annex'
                        ? 'Annex — a document with approved progress entries.'
                        : 'PDP is a plan that contains a list of skills/tasks to achieve.'
                "
            />
            ...
            >
                <div
                    class="w-full max-w-lg rounded-xl border border-border bg-background p-4 shadow-xl"
                >
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <label class="mb-1 block text-xs font-medium"
                                >Progress entries</label
                            >
                            <div
                                class="max-h-64 divide-y overflow-auto rounded-md border"
                            >
                                <div
                                ...
                                <template v-else>
                                    <div
                                        v-if="
                                            progressState.entries.length === 0
                                        "
                                        class="p-3 text-xs text-muted-foreground"
                                    >
                                        No entries yet.
                                    ...
                                    <div
                                        v-for="e in progressState.entries"
                                        :key="e.id"
                                        class="p-3 text-sm"
                                    >
                                        <div
                                            ...
                                            >
                                                <button
                                                    v-if="
                                                        e.status ===
                                                            'pending' &&
                                                        selectedPdpIsCurator
                                                    "
                                                    class="rounded border px-2 py-0.5 text-[10px] hover:bg-muted"
                                                    @click="
                                                ...
                                                <button
                                                    ...
                                                    "
                                                    class="rounded border px-2 py-0.5 text-[10px] hover:bg-muted"
                                                    @click="editNoteFor(e.id)"
                                                >
                                                    Edit
                                                </button>
                                                <button
                                                    v-if="selectedPdpIsOwner"
                                                    class="rounded border px-2 py-0.5 text-[10px] text-destructive hover:bg-destructive hover:text-destructive-foreground"
                                                    @click="
                                                        deleteProgressEntry(
                                                            e.id,
                                                        )
                                        ...
                                        <div
                                            v-if="
                                                progressState.editingNoteId ===
                                                    e.id && selectedPdpIsOwner
                                            "
                                            class="mt-2"
                                        >
                                            <textarea
                                            ...
                                            ></textarea>
                                            <div
                                                class="mt-1 flex justify-end gap-1"
                                            >
                                                <button
                                                    class="rounded border px-2 py-0.5 text-[10px] hover:bg-muted"
                                                    @click="saveOwnerNote(e.id)"
                                                >
                                                    Save
                                                </button>
                                                <button
                                        ...
                                        <div
                                            v-if="
                                                progressState.editingCommentId ===
                                                    e.id && selectedPdpIsCurator
                                            "
                                            class="mt-2"
                                        >
                                            <textarea
                                        ...
                                        <div
                                            v-else-if="e.curator_comment"
                                            class="mt-2 rounded-md bg-muted/50 px-2 py-1 text-xs"
                                        >
                                            <div
                        ...
                        <div v-if="selectedPdpIsOwner">
                            ...
                            >
                            <textarea
                                v-model="progressState.newNote"
                                rows="3"
                                class="w-full rounded-md border bg-transparent px-3 py-2 text-sm"
                                placeholder="Describe what was done / intermediate result"
                            ></textarea>
