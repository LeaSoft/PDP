...
const overloaded = ref<number | null>(
    new URLSearchParams(location.search).get('overloaded')
        ? Number(new URLSearchParams(location.search).get('overloaded'))
        : null,
);
...
function rowClass(r: Row) {
    if (r.pending_entries_count > 0) return 'bg-rose-50 dark:bg-rose-950/35';
    return '';
}
