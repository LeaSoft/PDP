...
const olderThan = ref<number | null>(
    new URLSearchParams(location.search).get('older_than')
        ? Number(new URLSearchParams(location.search).get('older_than'))
        : null,
);
const curatorId = ref<number | null>(
    new URLSearchParams(location.search).get('curator_id')
        ? Number(new URLSearchParams(location.search).get('curator_id'))
        : null,
);
...
function rowClass(r: Row) {
    if (r.status === 'pending' && r.days_pending > 10)
        return 'bg-rose-50 dark:bg-rose-950/35';
    if (r.status === 'pending' && r.days_pending > 0)
        ...
        <div class="mb-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 text-xs">
                <label
                    >Status
                    <select
                        class="ml-1 rounded border border-border/80 bg-background px-2 py-1 text-foreground"
                        v-model="status"
                        @change="
                            pageNum = 1;
                            syncUrl();
                ...
                <label
                    >Older than (days)
                    <input
                        class="ml-1 w-20 rounded border border-border/80 bg-background px-2 py-1 text-foreground"
                        type="number"
                        min="0"
